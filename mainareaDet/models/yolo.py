...
from .common import *
# from common import * # for export model
...
class Detect(nn.Module):
    ...
    def forward(self, x:List[Tensor]):
        ...
        # return (x, self.grid, self.anchor_grid, self.stride, self.no, self.anchors) 
        # --------------------------------for offline-------------------
        for i, m in enumerate(self.m):
            x[i] = m(x[i])  # conv
        return x
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                xy4, conf = x[i].split((8, self.no-8), 4)
                conf = conf.sigmoid()
                xy4  = (xy4 + self.grid[i].repeat((1, 1, 1, 1, 4))) * self.stride[i]
                y    = torch.cat((xy4, conf), 4)

                # -----------NCNN, TensorRT 不支持切片操作
                # y = x[i].clone()
                # y[..., 8:] = y[..., 8:].sigmoid() 
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=8, anchors=None):  # model, input channels, number of classes
        super().__init__()
        ...
        self.yolobody, self.det, self.save = parse_model(deepcopy(self.yaml), ch=[ch])  # model, savelist
    ...
    def forward(self, x):
        ...
        y = []  # outputs
        for i, m in enumerate(self.yolobody):
            if m.f != -1:  # if not from previous layer
                if isinstance(m.f, int):
                    x = y[m.f] 
                else:
                    x = torch.cat([x if j == -1 else y[j] for j in m.f], 1)  # from earlier layers

            x = m(x) # run
            y.append(x)  # save output
            # y.append(x if m.i in self.save else None)  # save output
        
        x1 = [x if j == -1 else y[j] for j in self.det.f]
        x = self.det(x1)
        return x
    ...
    def _initialize_biases(self, m, cf=None):  # initialize biases into Polygon_Detect(), cf is class frequency
...
class Polygon_Model(Model):
    ...
    def nms(self, mode=True):  # add or remove Polygon_NMS module
        ...
        if mode and not present:
            ...
            print('Removing Polygon_NMS... ')
            self.model = self.model[:-1]  # remove
        return self

    def autoshape(self):  # add Polygon_AutoShape module
        print('Adding Polygon_AutoShape... ')
...
def parse_model(d, ch, polygon=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 ShuffleV2Block, StemBlock, C3, C3TR]:
            c1, c2 = ch[f], args[0]
    ...
    bodylayers = layers[:-1]
    detlayer   = layers[-1]
    return nn.Sequential(*bodylayers), detlayer, sorted(save)
