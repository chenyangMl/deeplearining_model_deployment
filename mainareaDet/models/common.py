...
def channel_shuffle(x:Tensor, groups:Tensor):
    batchsize, num_channels, height, width = x.data.size()
    channels_per_group = num_channels // 2 # groups
    # channels_per_group =  torch.div(torch.tensor(num_channels), groups, rounding_mode='trunc')
...
class ShuffleV2Block(nn.Module):
    ...
    def forward(self, x):
        ...
        out = channel_shuffle(out, torch.tensor(2))
        return out
